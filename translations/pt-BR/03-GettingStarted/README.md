...
- **1 Seu primeiro servidor**, nesta primeira lição, você aprenderá como criar seu primeiro servidor e inspecioná-lo com a ferramenta inspetor, uma maneira valiosa de testar e depurar seu servidor, [para a lição](01-first-server/README.md)

- **2 Cliente**, nesta lição, você aprenderá como escrever um cliente que possa se conectar ao seu servidor, [para a lição](02-client/README.md)

- **3 Cliente com LLM**, uma maneira ainda melhor de escrever um cliente é adicionando um LLM para que ele possa "negociar" com seu servidor o que deve fazer, [para a lição](03-llm-client/README.md)

- **4 Consumindo o modo Agente do servidor GitHub Copilot no Visual Studio Code**. Aqui, vamos ver como executar nosso Servidor MCP dentro do Visual Studio Code, [para a lição](04-vscode/README.md)

- **5 Servidor de Transporte stdio** o transporte stdio é o padrão recomendado para comunicação local entre servidor MCP e cliente, fornecendo comunicação segura baseada em subprocessos com isolamento de processo incorporado [para a lição](05-stdio-server/README.md)
...
- **7 Utilizando o Kit de Ferramentas AI para VSCode** para consumir e testar seus Clientes e Servidores MCP [para a lição](07-aitk/README.md)

- **8 Testes**. Aqui vamos focar especialmente em como testar nosso servidor e cliente de diferentes maneiras, [para a lição](08-testing/README.md)
...
- **11 Autenticação**. Este capítulo aborda como adicionar autenticação simples, desde Autenticação Básica até o uso de JWT e RBAC. Você é encorajado a começar aqui e depois olhar os Tópicos Avançados no Capítulo 5 e realizar endurecimento adicional de segurança via recomendações no Capítulo 2, [para a lição](./11-simple-auth/README.md)

- **12 Hosts MCP**. Configure e use clientes hosts populares MCP incluindo Claude Desktop, Cursor, Cline e Windsurf. Aprenda tipos de transporte e solução de problemas, [para a lição](./12-mcp-hosts/README.md)

- **13 Inspetor MCP**. Faça a depuração e teste interativo dos seus servidores MCP usando a ferramenta Inspetor MCP. Aprenda a solucionar problemas com ferramentas, recursos e mensagens de protocolo, [para a lição](./13-mcp-inspector/README.md)

- **14 Amostragem**. Crie Servidores MCP que colaboram com clientes MCP em tarefas relacionadas a LLM (obsoleto no candidato à versão `2026-07-28`; ainda válido para `2025-11-25`). [para a lição](./14-sampling/README.md)

- **15 Aplicativos MCP**. Construa Servidores MCP que também respondem com instruções de interface de usuário, [para a lição](./15-mcp-apps/README.md)

O Protocolo de Contexto do Modelo (MCP) é um protocolo aberto que padroniza como os aplicativos fornecem contexto para LLMs. Pense no MCP como uma porta USB-C para aplicações de IA - ele fornece uma forma padronizada de conectar modelos de IA a diferentes fontes de dados e ferramentas.
...
Antes de começar a trabalhar com MCP, é importante preparar seu ambiente de desenvolvimento e entender o fluxo de trabalho básico. Esta seção irá guiá-lo pelos passos iniciais para garantir um início tranquilo com MCP.
...
- **Gerenciadores de Pacote**: NuGet, Maven/Gradle, pip ou npm/yarn
- **Chaves de API**: Para quaisquer serviços de IA que planeja usar em seus aplicativos host
